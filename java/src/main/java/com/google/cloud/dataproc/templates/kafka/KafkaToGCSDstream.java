...
public class KafkaToGCSDstream implements BaseTemplate {
        ...
        stream.foreachRDD(
                (VoidFunction2<JavaRDD<ConsumerRecord<Object, Object>>, Time>)
                        (rdd, time) -> {
                            ...
                            Dataset<Tuple2<String, String>> recordDataset =
                                    spark.createDataset(
                                            recordRdd.rdd(), Encoders.tuple(Encoders.STRING(), Encoders.STRING()));

                            DataFrameWriter<Tuple2<String, String>> writer =
                                    recordDataset.write().mode(gcsWriteMode).format(gcsOutputFormat);
    ...
    public void validateInput() throws IllegalArgumentException {
        ...
        LOGGER.info(
                "Starting Kafka to GCS Dstream spark job with following parameters:"
                        + "1. {}:{} "
                        + "2. {}:{} "
                        + "3. {}:{} "
                        + "4. {}:{} "
